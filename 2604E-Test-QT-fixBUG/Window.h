...
public:
    ...
    /// When set (e.g. the QMainWindow from createWindowContainer), windowW/windowH resize this widget;
...
protected:
    ...
    bool readDepthBufferFloat01(std::vector<float>& outDepth, uint32_t& outW, uint32_t& outH);

    bool readDepthBufferRgba8(std::vector<uint8_t>& outRgba, uint32_t& outW, uint32_t& outH);
...
private:
    ...
    glm::vec3 _bookmarkCoordinate = _coordinate;
    glm::vec2 _bookmarkRotation = _rotation;
    glm::vec4 _bookmarkOrtho = _ortho;
