...
/** Text drawn upright along a vertical strip (90° CW from normal reading). */
static QPixmap makeVerticalRotatedCaptionPixmap(const QString& text, int pixelSize) {
    QFont font;
    ...
    p.rotate(-90);
    p.drawText(QRect(-2 / 2, -th / 2, tw, th), Qt::AlignCenter, text);
    return pm;
}
...
MainWindow::MainWindow(Application* app, QVulkanInstance* vulkanInstance, QWidget* parent)
    ...
    {
        const QPixmap cap = makeVerticalRotatedCaptionPixmap(QStringLiteral("View"), 10);
    ...
    if (_app) {
        _app->setHostTopLevelWindow(this);
    }
}
...
void MainWindow::syncOrthoSpinsFromAppUnlessFocused() {
    if (!_app || !_orthoL || !_orthoR || !_orthoB || !_orthoT) {
        return;
    }
    const auto s = _app->viewStateSnapshot();
