...
void showDepthBufferViewer(QWidget* parent, MainWindow* win) {
    ...
    uint32_t w = 0;
    uint32_t h = 0;
    if (!win->captureDepthBufferFloat01(depth, w, h) || w == 0 || h == 0) {
        QMessageBox::warning(
            parent,
...
MainWindow::MainWindow(QWidget* parent)
    ...
    if (_autoHidePeekTab && _menuBar) {
        const QPixmap cap = makeVerticalCaptionPixmap(QStringLiteral("View"), _menuBar->font());
...
void MainWindow::attachAndInit() {
    attachQtWindow(vulkanWindow(), &_vulkanInstance);
    if (!_framePumpTimer) {
...
void MainWindow::pumpFrame() {
    ...
    updateReadoutLabel(QString::fromStdString(cursorReadoutString()));
}
...
bool MainWindow::captureDepthBufferFloat01(std::vector<float>& outDepth, uint32_t& outW, uint32_t& outH) {
    drawFrame();
