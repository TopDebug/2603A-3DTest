...
void VulkanWindow::saveBookmark() {
...
bool VulkanWindow::restoreBookmark() {
    if (!_hasBookmark) {
        return false;
    }
...
bool VulkanWindow::readDepthBufferFloat01(std::vector<float>& outDepth, uint32_t& outW, uint32_t& outH) {
...
bool VulkanWindow::readDepthBufferRgba8(std::vector<uint8_t>& outRgba, uint32_t& outW, uint32_t& outH) {
    std::vector<float> d;
...
glm::vec2 VulkanWindow::getRotation() const {
    return _rotation;
}

void VulkanWindow::setRotation(const glm::vec2& rotationDeg) {
