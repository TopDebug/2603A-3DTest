...
glm::vec4 VulkanWindow::buildRulerOrtho() const {
    ...
    const float Ox = glm::dot(-_coordinate, viewX);
    const float Oy = glm::dot(-_coordinate, viewY);

    glm::vec4 rulerOrtho;
    rulerOrtho[0] = _ortho[0] - Ox;
    rulerOrtho[1] = _ortho[1] - Ox;
    rulerOrtho[2] = _ortho[2] - Oy;
    rulerOrtho[3] = _ortho[3] - Oy;
...
void VulkanWindow::cursorPosCallbackImpl(double xpos, double ypos) {
    ...
    if (shiftHeld) { // Pan (translate target) when shift is held
        ...
        float transY = deltaY * (_ortho[2] - _ortho[3]) / _viewport[1];
...
void VulkanWindow::scrollCallbackImpl(float yoffset) {
    ...
    const bool altHeld = km.testFlag(Qt::AltModifier) || km.testFlag(Qt::MetaModifier);
    if (altHeld) {
        float transZ = static_cast<float>(-yoffset) * fabs(_ortho[3] - _ortho[2]) / _viewport[1] * 20;
...
void VulkanWindow::updateUniformBuffer(uint32_t currentImage) {
    ...
    ubo.proj = proj;
/*
    glm::mat4 proj2 = glm::ortho(_ortho[0], _ortho[1], _ortho[2], _ortho[3], 0.1f, 200.0f);
    //proj2[1][1] *= -1;
    //proj2[3][1] *= -1;
    ...
    for(int i = 0; i < 4; i++){
        for(int j = 0; j < 4; j++){
            data[i *4 + j] = proj[i][j];
            data2[i *4 + j] = proj2[i][j];
        }
    }

    for(int i = 0; i < 16; i++){
        if(fabs(data[i] - data2[i]) > 1e-6f){
            printf("%d: %f, %f\n", i, data[i], data2[i]);
        }
    }
    ...
    for(int i = 0; i < 4; i++){
        for(int j = 0; j < 4; j++){
            if(fabs(proj[i][j] - proj2[i][j]) > 1e-6f){
                printf("%d, %d: %f, %f\n", i, j, proj[i][j], proj2[i][j]);
            }
        }
    }
*/
    ...
    rulerUbo.proj = glm::ortho(rulerOrtho[0], rulerOrtho[1], rulerOrtho[3], rulerOrtho[2], 0.1f, 200.0f);
