...
static void appendVulkanRulerVertices(
    ...
    // Mouse world coordinate text at left-bottom ("<x> <y>"), Vulkan line glyphs.
    const float clampedMouseX = (mouseX < 0.0f) ? 0.0f : ((mouseX > safeW) ? safeW : mouseX);
    const float clampedMouseY = (mouseY < 0.0f) ? 0.0f : ((mouseY > safeH) ? safeH : mouseY);
    const float mouseWorldX = L + (clampedMouseX / safeW) * (R - L);
    const float mouseWorldY = T - (clampedMouseY / safeH) * (T - B);
    char mouseBuf[256] = {};
    if (hasDepthWorld) {
        std::snprintf(
    ...
    const std::string mouseText = std::string(mouseBuf);
    // Place cursor readout at top-right, just below the menu bar so ImGui doesn't cover it.
    const float mouseTextRightX = R - 8.0f * worldPerPixelX;
    //const float mouseTextCenterY = B + 35.0f * worldPerPixelY;
    const float mouseTextCenterY = B + 15.0f * worldPerPixelY;
    appendLabelSegmentsRightAligned(
        out, mouseText, mouseTextRightX, mouseTextCenterY,
        charW, charH, charSpacing, textColor);
}
...
void Window::createVertexBuffer() {
    ...
    float depth01 = 1.0f;
    const bool hasDepthWorld = readDepthAtCursor(_lastX, _lastY, depth01);
    const glm::vec3 depthWorld = hasDepthWorld
        ? screenToWorldByDepth(_lastX, _lastY, depth01)
...
std::string Window::getCursorReadoutText() {
    const float safeW = (_swapChainExtent.width > 0) ? static_cast<float>(_swapChainExtent.width) : 1.0f;
    const float safeH = (_swapChainExtent.height > 0) ? static_cast<float>(_swapChainExtent.height) : 1.0f;
    const float clampedMouseX = static_cast<float>(std::clamp<double>(_lastX, 0.0, safeW));
    const float clampedMouseY = static_cast<float>(std::clamp<double>(_lastY, 0.0, safeH));
