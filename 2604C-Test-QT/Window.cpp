...
static void appendGlyphSegments(
    ...
    float w, float h,
    const glm::vec3& color) {
    const auto fy = [](float y) { return 1.0f - y; };
    const auto seg = [&](bool on, float x0, float y0, float x1, float y1) {
        if (on) {
            appendLineSegment(out, ox + x0 * w, oy + fy(y0) * h, ox + x1 * w, oy + fy(y1) * h, color);
        }
    };
    ...
    case '.':
        ...
        appendLineSegment(out, ox + 0.17f * w, oy + fy(0.02f) * h, ox + 0.83f * w, oy + fy(0.02f) * h, color);
        return;
    case ',':
        appendLineSegment(out, ox + 0.45f * w, oy + fy(0.02f) * h, ox + 0.55f * w, oy + fy(-0.10f) * h, color);
        return;
    case '(':
        appendLineSegment(out, ox + 0.60f * w, oy + fy(1.00f) * h, ox + 0.40f * w, oy + fy(0.50f) * h, color);
        appendLineSegment(out, ox + 0.40f * w, oy + fy(0.50f) * h, ox + 0.60f * w, oy + fy(0.00f) * h, color);
        return;
    case ')':
        appendLineSegment(out, ox + 0.40f * w, oy + fy(1.00f) * h, ox + 0.60f * w, oy + fy(0.50f) * h, color);
        appendLineSegment(out, ox + 0.60f * w, oy + fy(0.50f) * h, ox + 0.40f * w, oy + fy(0.00f) * h, color);
        return;
    case ' ':
        return;
    default:
...
static void appendVulkanRulerVertices(
    ...
    const float bottomY = T - kAxisInsetPx * worldPerPixelY;
...
void Window::attachQtWindow(QWindow* window, QVulkanInstance* vulkanInstance) {
    _qtWindow = window;
...
void Window::getQtWindowSize(int& outW, int& outH) const {
    if (!_qtWindow) {
        outW = 0;
        outH = 0;
        return;
...
void Window::getQtFramebufferSize(int& outW, int& outH) const {
    if (!_qtWindow) {
        outW = 0;
        outH = 0;
        return;
...
bool Window::handleQtEvent(QObject* /*watched*/, QEvent* event) {
    switch (event->type()) {
    case QEvent::Resize:
    ...
    case QEvent::KeyPress: {
        ...
        if (e->key() == Qt::Key_F2) {
            saveBookmark();
            std::cout << "Bookmark saved (F2).\n";
            return true;
        }
        if (e->key() == Qt::Key_F3) {
            if (restoreBookmark()) {
                std::cout << "Bookmark restored (F3).\n";
            }
            else {
                std::cout << "No bookmark to restore. Press F2 first.\n";
            }
            return true;
        }
        return false;
...
void Window::saveBookmark() {
    _bookmarkCoordinate = _coordinate;
    _bookmarkRotation = _rotation;
    _bookmarkOrtho = _ortho;
    _hasBookmark = true;
}

bool Window::restoreBookmark() {
    if (!_hasBookmark) {
        return false;
    }
    _coordinate = _bookmarkCoordinate;
    setRotation(_bookmarkRotation);
    _ortho = _bookmarkOrtho;
    _overlayDirty = true;
    return true;
}
...
bool Window::loadViewConfig() {
    ...
    float viewportW = _viewport.x;
    float viewportH = _viewport.y;
    ...
    const bool ok =
        ...
        readFloat("viewportW", viewportW) &&
        readFloat("viewportH", viewportH);
...
bool Window::saveViewConfig() const {
    ...
    output << "        <viewportW>" << static_cast<int>(_viewport.x) << "</viewportW>\n";
    output << "        <viewportH>" << static_cast<int>(_viewport.y) << "</viewportH>\n";
    output << "    </View>\n";
    output << "</Config>\n";
...
bool Window::readDepthBufferFloat01(std::vector<float>& outDepth, uint32_t& outW, uint32_t& outH) {
    ...
    const VkDeviceSize rowBytes = static_cast<VkDeviceSize>(w) * bytesPerPixel;
    const VkDeviceSize bufferSize = rowBytes * static_cast<VkDeviceSize>(h);
    ...
    for (uint32_t y = 0; y < h; ++y) {
        for (uint32_t x = 0; x < w; ++x) {
            const size_t si = static_cast<size_t>(y) * static_cast<size_t>(rowBytes) + static_cast<size_t>(x) * 4;
            uint32_t packed = 0;
...
bool Window::readDepthBufferRgba8(std::vector<uint8_t>& outRgba, uint32_t& outW, uint32_t& outH) {
    std::vector<float> d;
    if (!readDepthBufferFloat01(d, outW, outH)) {
        return false;
    }
    outRgba.resize(d.size() * 4);
    for (size_t i = 0; i < d.size(); ++i) {
        const uint8_t g = static_cast<uint8_t>(std::lround(static_cast<double>(d[i]) * 255.0));
        outRgba[i * 4 + 0] = g;
        outRgba[i * 4 + 1] = g;
        outRgba[i * 4 + 2] = g;
        outRgba[i * 4 + 3] = 255;
...
bool Window::readDepthAtCursor(double mouseX, double mouseY, float& outDepth01) {
    ...
    // Flip Y to match Vulkan image coordinates used by depth image readback.
    const uint32_t pixelYTopLeft = static_cast<uint32_t>(std::clamp<int>(
        static_cast<int>(framebufferY), 0, framebufferHeight - 1));
    const uint32_t pixelY = static_cast<uint32_t>(framebufferHeight - 1) - pixelYTopLeft;
...
void Window::mouseButtonCallbackImpl(Qt::MouseButton button, QEvent::Type type, Qt::KeyboardModifiers mods, double localX, double localY, long long timestampMs) {
    _lastX = localX;
    _lastY = localY;
...
void Window::scrollCallbackImpl(float yoffset) {
    const Qt::KeyboardModifiers km = QGuiApplication::keyboardModifiers();
    const bool altHeld = km.testFlag(Qt::AltModifier) || km.testFlag(Qt::MetaModifier);
    if (altHeld) {
...
void Window::createRulerPipeline() {
    VkShaderModule vertModule = VK_NULL_HANDLE;
    VkShaderModule fragModule = VK_NULL_HANDLE;
#ifdef TEST_USE_PREBUILT_SPV_ONLY
...
void Window::createVertexBuffer() {
    ...
    float depth01 = 1.0f;
    bool hasDepthWorld = readDepthAtCursor(_lastX, _lastY, depth01);
...
void Window::updateUniformBuffer(uint32_t currentImage) {
    ...
    float scale = (_ortho[3] - _ortho[2]) * 0.5f;
    axesUbo.model = glm::scale(axesUbo.model, glm::vec3(scale, scale, scale));

    axesUbo.view = view;
    axesUbo.proj = proj;

    axesUbo.proj[1][1] *= -1; // flip Y for Vulkan NDC
  ...
  //  rulerUbo.proj[1][1] *= -1; // Vulkan NDC Y-flip
...
void Window::enforceOrthoAspectFromWindow() {
    int w = 0;
    int h = 0;
    getQtFramebufferSize(w, h);
    if (w <= 1 || h <= 1) {
...
void Window::setApplicationModelTransform(const glm::vec3& translation, const glm::vec3& scale) {
...
std::string Window::getCursorReadoutText() {
    int ww = 0;
    int wh = 0;
...
namespace {
float wrapDegrees0To360(float deg) {
...
void Window::setRotation(const glm::vec2& rotationDeg) {
    _rotation = glm::vec2(wrapDegrees0To360(rotationDeg.x), wrapDegrees0To360(rotationDeg.y));
}

void Window::setViewportSize(const glm::ivec2& size) {
